...
namespace SophisticatedStreamCam.Sceeps
{
    public class Streamered
    {
        ...
        public Streamered(RenderTexture initialTexture)
        {
            ...
            ffmpegProcess = new Process
            {
                StartInfo = new ProcessStartInfo
                {
                    ...
                    Arguments = $"-f rawvideo -pix_fmt rgba -s {activeRenderTexture.width}x{activeRenderTexture.height} " +
                                $"-i - -f dshow -i audio=\"Microphone (Default)\" " + // Capture default microphone
                                $"-c:v libx264 -preset veryfast -pix_fmt yuv420p " +
                                $"-c:a aac -b:a 160k -f flv rtmp://live.twitch.tv/app/{twitchStreamKey}",
